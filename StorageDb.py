...
class StorageDb(object):
    def __init__(self, db_filename):
        if (db_filename and db_filename.strip() and db_filename != ''):
            ...
            self.conn = sqlite3.connect(db_filename, timeout=1, check_same_thread=False)
            self.cursor = self.conn.cursor()
        else:
            # self.conn = sqlite3.connect('wfm.sqlite', timeout=1)
            self.conn = sqlite3.connect('wfm.sqlite', timeout=1, check_same_thread=False)
            self.cursor = self.conn.cursor()

    def CreateDefaultTable(self):
        self.CreateWFMTable('wfm')

    def CreateWFMTable(self, table):
        self.conn.executescript('''CREATE TABLE IF NOT EXISTS ''' + table + '''
              ...
              ApptDate VARCHAR (50),
              ApptSlot VARCHAR (50),
              ApptSlotFrom VARCHAR (50),
              ApptSlotTo VARCHAR (50),
              ...
              unique_ord_no VARCHAR (255) NOT NULL);''')

        print("Table created successfully")
        
    def CreateErrorLinksTable(self, table):
        self.conn.executescript('''CREATE TABLE IF NOT EXISTS ''' + table + '''
              (id INTEGER PRIMARY KEY AUTOINCREMENT UNIQUE NOT NULL,
              ord_no VARCHAR (50) NOT NULL,
              links TEXT);''')

    def CreateLinksTable(self, table):
        self.conn.executescript('''CREATE TABLE IF NOT EXISTS ''' + table + '''
              (id INTEGER PRIMARY KEY AUTOINCREMENT UNIQUE NOT NULL,
              unique_orderno VARCHAR (50) NOT NULL,
    ...
    def IsExist(self, table, column, value):
        ...
        query = "SELECT {0} FROM {1} WHERE {0} = '{2}';".format(column, table, value)
